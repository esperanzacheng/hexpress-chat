            ...
            for (let i = 0; i < allResult.length; i++) {
                for (let j = 0; j < user['friends'].length; j++) {
                    if ( allResult[i]._id.toString() == user.friends[j]._id.toString() ) {
                        const friendship = { 
                            _id: allResult[i]._id,
                            username: allResult[i].username,
                            profilePicture: await s3.getObjectSignedUrl(allResult[i].profilePicture),
                            verified: user.friends[j].verified,
                            sender: user.friends[j].sender,
                        }
